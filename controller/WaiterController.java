...
public class WaiterController {
    ...
    public void initialize() {
        ...
        if (RestaurantDatabase.tables.isEmpty()) {
            RestaurantDatabase.tables.add(new Table(1, 2, TableType.INDOOR));
            RestaurantDatabase.tables.add(new Table(2, 4, TableType.INDOOR));
            RestaurantDatabase.tables.add(new Table(3, 6, TableType.OUTDOOR));
        }
    ...
    public void handleClearTable(ActionEvent event) {
        if (currentlySelectedTable != null) {
            currentlySelectedTable.setStatus(TableStatus.AVAILABLE);
